...
PayPal credentials are server-only and must not use the `VITE_` prefix. In Vercel, add `PAYPAL_CLIENT_ID`, `PAYPAL_CLIENT_SECRET`, `PAYPAL_ENV`, `SUPABASE_SERVICE_ROLE_KEY`, and `VITE_PAYPAL_ENABLED` to the deployment environment, then redeploy. `PAYPAL_SECRET_KEY` remains supported as a temporary compatibility fallback.
