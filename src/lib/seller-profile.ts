...
export async function loadSellerProfile(sellerId: string) {
    ...
    const [{ data: profile, error: profileError }, { data: listings, error: listingsError }] = await Promise.all([
        ...
        supabase.from('listings').select('id, seller_id, name, price, description, listing_type, image, images, is_free_delivery, condition, review_status, status').eq('seller_id', sellerId).eq('is_active', true).eq('review_status', 'approved').neq('status', 'completed').order('created_at', { ascending: false }),
    ]);
