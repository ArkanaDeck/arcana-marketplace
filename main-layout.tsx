    ...
    >
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-[#E5E7EB] px-6 py-4 shadow-sm">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div onClick={() => setActiveTab('marketplace')} className="text-xl font-bold tracking-wider cursor-pointer">
            🔮 ARKANA
          </div>
          <div className="flex space-x-6">
            <button onClick={() => setActiveTab('marketplace')} className={`text-sm font-semibold ${activeTab === 'marketplace' ? 'text-[#D4AF37]' : 'text-gray-500'}`}>Marketplace</button>
            <button onClick={() => setActiveTab('checkout')} className={`text-sm font-semibold ${activeTab === 'checkout' ? 'text-[#D4AF37]' : 'text-gray-500'}`}>Basket & Checkout</button>
            <button onClick={() => setActiveTab('dashboard')} className={`text-sm font-semibold ${activeTab === 'dashboard' ? 'text-[#D4AF37]' : 'text-gray-500'}`}>Seller Dashboard</button>
          </div>
        </div>
      </nav>
